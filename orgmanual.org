...
#+attr_texinfo: :table-type "table" :indic "@asis"
  ...
  - {{{kbdkey(S-,up)}}} :: 
    @@info:@itemx@@ {{{kbdkey(S-,down)}}}
       ...
       Jump to the previous/next item in the current list,[fn:20] but
...
prefers[fn:25] to use another, more general representation that looks
like this:
...
you want to be able to assign /names/ [fn:30] to fields and columns,
...
#+attr_texinfo: :table-type "table" :indic "@asis"
  ...
  - {{{kbd(C-c C-l)}}} ~org-insert-link~ :: Insert a link.[fn:35] This
       ...
       will be inserted into the buffer,[fn:36] along with a
...
DONE,[fn:49] and that a note is recorded when switching to WAIT or
...
* Improvements and fixes [2/12]                                    :noexport:
- [ ] Jon will fix detailed node listing
- [ ] Jon will fix :INDEX: property
- [ ] New link type to generate pxref?
- [X] New macro for kbdkey that preserves space, e.g., `C-c <RET>'
- [ ] Indent examples one more space to match indentation of footnotes
- [ ] How to generate @kbd{\}?
- [ ] How to generate @kbd{~}?
- [ ] How to include GNU Free Documentation License?
- [X] Straighten out footnotes
- [ ] Truncated footnote
- [ ] Resolve macros with XXX arguments
- [ ] Get @appendix instead of @chapter?
