...
agenda]]), you can then use the {{{kbd(l)}}} key to display the TODO
items with a {{{samp(CLOSED)}}} timestamp on each day, giving you an
...
- {{{kbdkey(M-,TAB)}}} ::
  ...
  - After {{{samp(*)}}} :: 
    ...
    search links like {{{samp([[*find this headline]])}}}.
...
3. /Odd levels/
  ...
  double-star-per-level convention with {{{kbdkey(M-x org-convert-to-odd-levels, RET)}}} in that file. The reverse operation
...
{{{samp([[man:printf][The printf manpage]])}}} to show Unix manual pages inside Emacs:
...
* FIXED History and acknowledgments                                :noexport:
  :PROPERTIES:
