...
#+TEXINFO_HEADER: @set MAINTAINEREMAIL @email{bzg at gnu dot org}
#+TEXINFO_HEADER: @set MAINTAINERCONTACT @uref{mailto:bzg at gnu dot org,contact the maintainer}
...
- {{{key(TAB)}}}, ~org-cycle~ :: 
  ...
  Subtrees can be cycled through three states:
...
- {{{kbdkey(S-,TAB)}}} or {{{kbdspckey(C-u,TAB)}}}, ~org-global-cycle~ :: 
  ...
  Global cycling rotates the entire buffer among the states:
...
- {{{kbdkey(M-,RET)}}}, ~org-insert-heading~ :: 
  ...
  with a prefix argument.  When this command is used in the middle of
  a line, the line is split and the rest of the line becomes the new
  item or headline.[fn:11] If the command is used at the /beginning/
  of a headline, the new headline is created before the current line.
  ...
  Calling this command with ~C-u C-u~ will unconditionnally respect
...
- {{{kbd(M-h)}}}, ~org-mark-element~ ::
    ...
    subsequent elements of the one just marked.  E.g., hitting
    {{{kbd(M-h)}}} on a paragraph will mark it, hitting
    {{{kbd(M-h)}}} immediately again will mark the next one.

- {{{kbd(C-c @)}}}, ~org-mark-subtree~ ::

    Mark the subtree at point.  Hitting repeatedly will mark
    subsequent subtrees of the same level as the marked subtree.
...
#+attr_texinfo: :table-type table :indic @asis
  ...
  - {{{kbdkey(M-,left)}}} and {{{kbdkey(M-,right)}}} :: 
  ...
  - {{{kbdkey(M-S-,left)}}} and {{{kbdkey(M-S-,right)}}} :: 
...
*** Re-aligning and field motion
    ...
    :END:
#+attr_texinfo: :table-type table :indic @asis
- {{{kbd(C-c C-c)}}}, ~org-table-align~ :: Re-align the table and
     don't move to another field.
...
#+attr_texinfo: :table-type table :indic @code
  ...
  - @2$1..@4$3  ::  six fields between these two fields (same as
                   ~A2..C4~)
  - @-1$-2..@-1 :: three fiels in the row above, starting from two
                   columns on the left
...
#+attr_texinfo: :table-type table :indic @code
  ...
  - n3 s3 e2 f4 :: Set normal, scientific, engineering, or fixed format of
  ...
  - D R :: Set angle modes to degrees, radians.

  - F S :: Set fraction and symbolic modes.

  - E :: Keep empty fields in ranges and use ~nan~ (not a number) in
         Calc formulas for empty fields in range references and for
         empty field references; else suppress empty fields in range
         references and use 0 for empty field references, see also the
         notes for [[References][Range references]].

  - N :: Interpret all fields as numbers, use 0 for non-numbers; N has
         higher precedence than E (for the value of the field).

  - L :: Set literal mode, for Lisp formulas only.
...
#+attr_texinfo: :table-type table :indic @code
  ...
  - vmean($2..$7)    :: Compute column range mean, suppress empty fields
  - vmean($2..$7);EN :: Same, but treat empty fields as 0
  - taylor($3,x=7,2) :: Taylor series of $3, at x=7, second degree

Calc also contains a complete set of logical operations, e.g.:
...
#+attr_texinfo: :table-type table :indic @samp
  ...
  - w :: Unmarked lines are exempt from recalculation with {{{kbd(C-u C-c *)}}}.  All lines that should be recalculated should be
       marked with ~#~ or ~*~.
...
tables using [[http://www.gnuplot.info/][Gnuplot]] and [[http://xafs.org/BruceRavel/GnuplotMode][gnuplot-mode]]. To see this in action, ensure
...
#+attr_texinfo: :table-type table :indic @asis
  ...
  - {{{kbd(C-c C-x C-p)}}}, ~org-previous-link~ ::
       ...
       {{{cindex(links\\\, finding previous)}}}
       {{{kindex(C-c C-x C-p)}}}
       {{{findex(org-previous-link)}}}

       Move backward to the previous link in the buffer. At the
       limit of the buffer, the search fails once, and then wraps
       around. The key bindings for this are really too long; you
...
- {{{kbd(C-c C-t)}}}, ~org-todo~ ::
 ...
 states]]), you will be prompted for a TODO keyword through the fast
...
lightweight. Checkboxes are not included in the global TODO list, so
...
{{{noindent}}} To limit tag inheritance to specific tags, use
...
Unlike properties set from a special drawer, you have to refresh the
buffer with {{{kbd(C-c C-c)}}} to activate these changes.
...
- ~SCHEDULED~ ::
  ...
  want it to affect only the first scheduled occurrence of the task, use
  ~--2d~ instead.  See ~org-scheduled-delay-days~ and
  ...
  *Important:* Scheduling an item in Org mode should /not/ be
  ...
  on the date where it applies.  This is frequently misunderstood by
...
sexp function to issue early and late warnings.  However, it will show
the item on each day where the sexp entry matches.
...
by John Wiegley's excellent {{{file(remember.el)}}} package. Up to
version 6.36 Org used a special setup for {{{file(remember.el)}}},
then replaced it with {{{file(org-remember.el)}}}.  As of Org version
8.0, {{{file(org-remember.el)}}} has been completely replaced by
{{{file(org-capture.el)}}}. 

If your configuration depends on {{{file(org-remember.el)}}}, you need
to update it and use the setup described below.  To convert your
~org-remember-templates~, run the command {{{kbdspckey(M-x
org-capture-import-remember-templates,RET)}}} and then customize the
new variable with {{{kbd(M-x customize-variable
...
- {{{kbd(C-x C-w)}}}, ~org-agenda-write~ ::
  ...
  ({{{file(.html)}}} or {{{file(.htm)}}}), Postscript ({{{file(.ps)}}}),
  PDF ({{{file(.pdf)}}}), Org ({{{file(.org)}}}. and plain text (any
  other extension).  When exporting to Org, only the body of original
  headlines are exported, not subtrees or inherited tags.  When called
  with a {{{kbd(C-u)}}} prefix argument, immediately open the newly
  created file. Use the variable ~org-agenda-exporter-settings~ to set
  options for {{{file(ps-print)}}} and for {{{file(htmlize)}}} to be
  used during export.
...
{{{noindent}}} If this line does not exist, the title will be the name
of the file associated with the Org buffer, without extension, or the
buffer name.
...
location, insert ~#+TOC: headlines~ at the desired location.  The depth
of the table of contents is by default the same as the number of
headline levels, but you can choose a smaller number, or turn off the
table of contents entirely, by configuring the variable
~org-export-with-toc~, or on a per-file basis with a line like one of
...
title by setting the ~ALT_TITLE~ property accordingly.  It will then
be used when building the table.
...
{{{vindex(org-emphasis-regex-components)}}}
...
~org-fontify-emphasized-text~ to ~nil~.  To fine tune which characters
are allowed before and after the special characters, see
...
Some backends (HTML and LaTeX) allow you to directly
...
that can be displayed in a browser.[fn:110]

LaTeX fragments don't need any special marking at all.  The following
snippets will be identified as LaTeX source code:

- Environments of any kind.[fn:111] The only requirement is that the
  ~\begin~ and ~\end~ statements each appear on a new line, at the
...
means {{{file(MathJax)}}} for HTML, and no processing for
...
To disable it, simply use the following line:
...
version of an Org file.  HTML export allows you to publish a notes file
on the web.  LaTeX export lets you use Org mode and its structured
editing functions to easily create LaTeX files.  OpenDocument Text
(ODT) export allows seamless collaboration across organizational
boundaries.  To incorporate entries with associated times like
...
- #+BIND: ::       
  ...
  Lisp-var lisp-val, e.g., org-latex-low-levels itemize. You need
...
The default value for ~org-html-postamble~ is
...
~org-html-validation-link~ and build the postamble from these
values. Setting ~org-html-postamble~ to ~t~ will insert the
postamble from the relevant format string found in
~org-html-postamble-format~. Setting it to ~nil~ will not
...
~org-html-table-tag~. The default setting makes tables without
...
~org-html-mathjax-options~ or insert something like the
...
customizing the variable ~org-html-infojs-options~. If you always want to
...
Org mode contains a LaTeX exporter. With further processing, this
backend is also used to produce PDF output.[fn:124] Since the LaTeX
output uses {{{file(hyperref)}}} to implement links and cross
...
(sub)tree. The class must be listed in ~org-latex-classes~.
...
header. See the docstring of ~org-latex-classes~ for more
...
~org-latex-publish-to-pdf~, or as ~ascii~, ~latin1~ or
...
/tag-excluded/ trees removed, use ~org-org-publish-to-org~ and set the
...
{{{vindex(org-html-table-tag)}}}
{{{vindex(org-html-expand)}}}
{{{vindex(org-html-with-timestamp)}}}
...
#+attr_texinfo: :table-type table :indic @asis
- ~:link-up~ :: ~org-html-link-up~
- ~:link-home~ :: ~org-html-link-home~
...
- ~:style-include-default~ :: ~org-html-head-include-default-style~
- ~:style-include-scripts~ :: ~org-html-style-include-scripts~
- ~:style~ :: ~org-html-style~
- ~:style-extra~ :: ~org-html-style-extra~
- ~:convert-org-links~ :: ~org-html-link-org-files-as-html~
- ~:inline-images~ :: ~org-html-inline-images~
- ~:html-extension~ :: ~org-html-extension~
- ~:html-preamble~ :: ~org-html-preamble~
- ~:html-postamble~ :: ~org-html-postamble~
- ~:xml-declaration~ :: ~org-html-xml-declaration~
- ~:html-table-tag~ :: ~org-html-table-tag~
- ~:expand-quoted-html~ :: ~org-html-expand~
- ~:timestamp~ :: ~org-html-with-timestamp~
- ~:publishing-directory~ :: ~org-export-publishing-directory~
- ~:select-tags~ :: ~org-export-select-tags~
- ~:exclude-tags~ :: ~org-export-exclude-tags~
- ~:latex-image-options~ :: ~org-latex-image-default-option~
...
- ~org~ ::
  ...
  Include text from the Org mode file as a comment.  The text is picked
  from the leading context of the tangled code and is limited by the
  nearest headline or source block as the case may be.
...
- ~no~ ::
  ...
  on the default value yields the following results.  Note that the
  ~:hline~ header argument has no effect for Emacs Lisp code blocks.
...
with a default value of ~no~.  Note that Emacs Lisp code +blocks
ignore the ~:rownames~ header argument entirely given the ease
with which tables with row names may be handled directly in Emacs
Lisp.
...
- {{{kbd(#+TITLE:)}}}, {{{kbd(#+AUTHOR:)}}}, {{{kbd(#+EMAIL:)}}}, {{{kbd(#+LANGUAGE:)}}}, {{{kbd(#+TEXT:)}}}, {{{kbd(#+DATE:)}}}, {{{kbd(#+OPTIONS:)}}}, {{{kbd(#+BIND:)}}}, {{{kbd(#+DESCRIPTION:)}}}, {{{kbd(#+KEYWORDS:)}}}, {{{kbd(#+LaTeX_HEADER:)}}}, {{{kbd(#+HTML_HEAD:)}}}, {{{kbd(#+HTML_LINK_UP:)}}}, {{{kbd(#+HTML_LINK_HOME:)}}}, {{{kbd(#+SELECT_TAGS:)}}}, {{{kbd(#+EXCLUDE_TAGS:)}}}
...
- {{{file(table.el)}}} by Takaaki Ota ::
  ...
  - @@info:@kbd{C-c ~}@@, ~org-table-create-with-table.el~ ::
...
Pressing {{{kbd(C-c C-c)}}} on ~a new house~ will insert the converted
...
agenda commands.
  ...
  1. Reduce the number of Org agenda files: this will reduce the
     slowness caused by accessing a hard drive.
...
agenda generation, and this dedicated [[http://orgmode.org/worg/agenda-optimization.html][Word page]] for further
explanation.
...
- Achim Gratz ::
  ...
  with the many hiccups that such a change can create for users.
...
- Baoqiu Cui contributed the DocBook exporter, which was deleted from
  Org 8.0.  You can now export to Texinfo and export the
  {{{file(.texi)}}} file to DocBook using ~makeinfo~.
...
-  Christian Egli converted the documentation into Texinfo format,
   ...
   and wrote {{{file(org-taskjuggler.el)}}}, which has been rewritten
   by Nicolas Goaziou as {{{file(ox-taskjuggler.el)}}} for Org 8.0.
...
- Nicolas Goaziou rewrote much of the plain list code.  He also wrote
  `org-element.el' and `org-export.el', which were huge steps forward
...
* Improvements and fixes [8/12] 				   :noexport:
- [X] Jon will fix detailed node listing
- [X] Jon will fix :INDEX: property
- [X] New link type to generate pxref? (asked on ML)
...
* Bastien's instructions
~$ cd ~/git/org-mode
~$ git checkout master
~$ git pull
~$ git log --since="2013-01-01" --no-merges doc/org.texi
[Get the last commit in this log, e.g. xxxxxx]
~$ git diff xxxxxx.. doc/org.texi
...
~contrib/scripts~ directory of the Org git distribution, or install
the {{{file(xprintidle)}}} package and set it to the variable
~org-clock-x11idle-program-name~ if you are running Debian, to get the
same general treatment of idleness.  On other systems, idle time
...
Refer to ~org-latex-listings~ documentation for details.
...
[fn:120] But see the variable ~org-html-inline-images~.
...
~org-html-style-default~, which you should not modify. To turn
...
[fn:185] The {{{file(local.mk)}}} template is designed to work out of
the box with GNU systems.
