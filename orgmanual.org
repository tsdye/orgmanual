...
You can make words @b{*bold*}, //italic//, _underlined_, @code{=code=}
...
You can also add a @code{-r} switch which /removes/ the labels from the
source code@footnote{Adding @code{-k} to @code{-n -r} will /keep/ the
...
/tag-excluded/ trees removed, use @code{org-publish-org-to-org} and set the
...
@item
/Frank Ruell/ solved the mystery of the @code{keymapp nil} bug, a
