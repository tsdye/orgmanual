...
{{{kbdkey(C-,SPC)}}} twice before moving the cursor.
...
on document structure, {{{kbdkey(M-,right)}}} will be listed to call
...
- {{{key(TAB)}}} ~org-cycle~ :: /Subtree Cycling/: Rotate
...
- {{{kbdkey(C-u C-u C-u ,TAB)}}} ~show-all~ :: Show all, including drawers.
