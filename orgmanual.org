...
#+MACRO: kbd @@info:@kbd{@@$1@@info:}@@
#+MACRO: key @@info:@key{@@$1@@info:}@@
#+MACRO: command @@info:@command{@@$1@@info:}@@
#+MACRO: file @@info:@file{@@$1@@info:}@@

#+MACRO: orgcmd {{{kbd($1)}}} ~$2~
...
{{{vindex(org-insert-mode-line-in-empty-file)}}} @@info:@noindent@@
...
error occurred.  Here is how to produce a useful backtrace:
  ...
  1. Reload uncompiled versions of all Org mode Lisp files.  The
     ...
     uncompiled code. To do this, use {{{key(C-u M-x org-reload
     RET)}}} or select ~Org -> Refresh/Reload -> Reload Org uncompiled~
     from the menu.
...
unsetting the flag ~cmdnames~ in
{{{file(org.texi)}}}.
* Document structure
  :PROPERTIES:
...
{{{command(org-cycle)}}}, which is bound to the {{{key(TAB)}}}
key.
...
- {{{orgcmd({{{key(TAB)}}},org-cycle)}}} :: /Subtree Cycling/: Rotate
     current subtree among the states 
     {{{kindex(TAB)}}}
     {{{findex(org-cycle)}}}
     ...
     when called with a prefix argument ({{{kbd(C-u @key{TAB})}}}),
...
- {{{orgcmd(S-@key{TAB},org-global-cycle)}}} :: 
     ...
     be shown. Note that inside tables, {{{kbd(S-@key{TAB})}}} jumps
...
- {{{orgcmd(C-c C-k,show-branches)}}} :: Expose all the headings of
...
- {{{orgcmd(C-c C-x b,org-tree-to-indirect-buffer)}}} :: Show the
...
- {{{orgcmd(C-c C-x v,org-copy-visible)}}} :: Copy the /visible/ text
...
~all~.
#+attr_texinfo: :indic "@asis"
- {{{kbd(C-u C-u @key{TAB})}}} ~org-set-startup-visibility~ :: Switch
     back to the startup visibility of the buffer, i.e.@: whatever is
     requested by startup options and @@info:@samp{VISIBILITY}@@
