...
#+attr_texinfo: :table-type "table" :indic "@asis"
  - {{{code(@@2$3)}}} :: 2nd row, 3rd column (same as {{{code(C2)}}})
  - {{{code($5)}}} :: column 5 in the current row (same as {{{code(E&)}}})
  - {{{code(@@2)}}} :: current column, row 2
  - {{{code(@@-1$-3)}}} :: the field one row up, three columns to the left
  - {{{code(@@-I$2)}}} :: field just under hline above current row, column 2
  - {{{code(@@>$5)}}} :: field in the last row, in column 5
...
#+attr_texinfo: :table-type "table" :indic "@asis"
  - {{{code($1..$3)}}} :: first three fields in the current row
  - {{{code($P..$Q)}}} :: range, using column names (see under Advanced)
  - {{{code($<<<..$>>)}}} :: start in third column, continue to the one but last
  - {{{code(@@2$1..@@4$3)}}} ::  six fields between these two fields (same as {{{code(A2..C4)}}})
  - {{{code(@@-1$-2..@@-1)}}} :: three numbers from the column to the left, 2 up to current row
  - {{{code(@@I..II)}}} ::  between first and second hline, short for {{{code(@@I..@@II)}}}
...
#+attr_texinfo: :table-type "table" :indic "@asis"
  - {{{code(if(@@# % 2, $#, string("")))}}} :: column number on odd lines only
  - {{{code($3 = remote(FOO, @@@@#$2))}}} ::  copy column 2 from table FOO into column 3 of the current table
...
#+attr_texinfo: :table-type "table" :indic "@asis"
  - {{{code(p20)}}} :: set the internal Calc calculation precision to 20 digits
  - {{{code(n3 s3 e2 f4)}}} :: Normal, scientific, engineering, or
  ...
  - {{{code(D R)}}} :: angle modes: degrees, radians
  - {{{code(F S)}}} :: fraction and symbolic modes
  - {{{code(N)}}} :: interpret all fields as numbers, use 0 for non-numbers
  - {{{code(E)}}} :: keep empty fields in ranges
  - {{{code(L)}}} :: literal
...
#+attr_texinfo: :table-type "table" :indic "@asis"
  - {{{code($1+$2)}}} :: Sum of first and second field
  - {{{code($1+$2;%.2f)}}} :: Same, format result to two decimals
  - {{{code(exp($2)+exp($1))}}} :: Math functions can be used
  - {{{code($0;%.1f)}}} :: Reformat current cell to 1 decimal
  - {{{code(($3-32)*5/9)}}} :: Degrees F -> C conversion
  - {{{code($c/$1/$cm)}}} :: Hz -> cm conversion, using {{{file(constants.el)}}}
  - {{{code(tan($1);Dp3s1)}}} :: Compute in degrees, precision 3, display SCI 1
  - {{{code(sin($1);Dp3%.1e)}}} ::  Same, but use {{{code(printf)}}}
       specifier for display
  - {{{code(vmean($2..$7))}}} :: Compute column range mean, using vector function
  - {{{code(vmean($2..$7);EN)}}} :: Same, but treat empty fields as 0
  - {{{code(taylor($3,x=7,2))}}} :: Taylor series of $3, at x=7, second degree

Calc also contains a complete set of logical operations.  For example

#+attr_texinfo: :table-type "table" :indic "@asis"
  - {{{code(if($1<20,teen,string("")))}}} ::  "teen" if age $1 less than 20, else empty
...
*** Field and range formulas                                       :noexport:
    :PROPERTIES:
...
@table @kbd
{{{orgcmd(C-u C-c =,org-table-eval-formula)}}}Install a new formula for the current field.  The command prompts for a
...
*** Editing and debugging formulas                                 :noexport:
    :PROPERTIES:
...
{{{orgcmd(C-c ',org-table-edit-formulas)}}}Edit all formulas for the current table in a special buffer, where the
...
(defun tsd-simple-command-macros ()
  ...
  (interactive)
  (query-replace-regexp "@\\([tsubheading|pxref|samp|code|file|key|kbd]+\\){\\([^}]*\\)}" "{{{\\1(\\2)}}}" nil))
